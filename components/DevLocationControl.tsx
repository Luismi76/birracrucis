...
export default function DevLocationControl({
    ...
    const currentRounds = activeStop ? (rounds[activeStop.id] || 0) : 0;
    ...
    return (
        <div className="fixed bottom-24 right-4 z-50 pointer-events-auto">
            ...
            {isExpanded && (
                <div className="bg-slate-800 text-white rounded-2xl shadow-xl mb-2 p-3 w-64 animate-in slide-in-from-bottom-2 duration-200">
                    ...
                    <div className="space-y-1 mb-3">
                        ...
                        {/* DISCOVERY MODE TEST BUTTON */}
                        <button
                            onClick={() => onSetPosition({ lat: 40.4206, lng: -3.6997 })}
                            className="w-full text-left px-2 py-1.5 rounded-lg text-xs flex items-center gap-2 transition-all bg-amber-600 hover:bg-amber-500 text-white"
                        >
                            <MapPin className="w-3 h-3" />
                            <span className="truncate flex-1">Ir a El Tigre (Test)</span>
                        </button>
                        ...
                        {stops.map((stop, idx) => (
                            <button
                                ...
                                className={`w-full text-left px-2 py-1.5 rounded-lg text-xs flex items-center gap-2 transition-all ${idx === currentBarIndex
                    ...
                    {activeStop && onSetRounds && (
                        <div className="mb-3">
                            ...
                            <div className="flex items-center gap-2 bg-slate-700 rounded-lg p-2">
                                <button
                                    onClick={() => onSetRounds(activeStop.id, Math.max(0, (rounds[activeStop.id] || 0) - 1))}
                                    className="p-1.5 bg-slate-600 rounded hover:bg-slate-500 transition-colors"
                                    disabled={(rounds[activeStop.id] || 0) <= 0}
                                >
                                    <Minus className="w-4 h-4" />
                                </button>
                                <div className="flex-1 text-center">
                                    <span className="text-lg font-bold">{rounds[activeStop.id] || 0}</span>
                                    <span className="text-slate-400 text-sm">/{activeStop.plannedRounds}</span>
                                </div>
                                <button
                                    onClick={() => onSetRounds(activeStop.id, (rounds[activeStop.id] || 0) + 1)}
                                    className="p-1.5 bg-slate-600 rounded hover:bg-slate-500 transition-colors"
                                >
                    ...
                    <div className="space-y-1">
                        ...
                        {activeStop && stops[currentBarIndex + 1] && (
                            <button
                                onClick={() => onSetPosition({ lat: stops[currentBarIndex + 1].lat, lng: stops[currentBarIndex + 1].lng })}
                                className="w-full px-2 py-1.5 bg-blue-600 rounded-lg text-xs flex items-center gap-2 hover:bg-blue-500"
                            >
                                <Navigation className="w-3 h-3" />
                                Ir a siguiente bar ({stops[currentBarIndex + 1].name})
                            </button>
                        )}
                        {/* Simular fin de ruta */}
                        {stops.length > 0 && (
                    ...
                    <div className="mt-3 pt-2 border-t border-slate-700 text-[10px] text-slate-400">
                        ...
                        {activeStop && (
                            <div>Rondas: {rounds[activeStop.id] || 0}/{activeStop.plannedRounds} {(rounds[activeStop.id] || 0) >= activeStop.plannedRounds && activeStop.plannedRounds > 0 && '✓'}</div>
                        )}
                    </div>
                </div>
            )}
            ...
            <button
                ...
                className={`p-3 rounded-full shadow-lg transition-all flex items-center gap-2 ${isExpanded ? 'bg-slate-700 text-white' : 'bg-purple-600 text-white hover:bg-purple-700'
                    }`}
                title="Dev Tools"
            >
