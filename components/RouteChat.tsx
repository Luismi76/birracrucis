...
export default function RouteChat({ routeId, currentUserId, onClose, onUnreadCountChange }: RouteChatProps) {
  ...
  }, [messages.length, lastReadCount, onUnreadCountChange]);
  ...
  return (
    <div className="fixed inset-0 z-50 pointer-events-auto flex flex-col bg-white dark:bg-slate-900 md:inset-auto md:bottom-20 md:right-4 md:w-96 md:h-[500px] md:rounded-2xl md:shadow-2xl md:border md:border-slate-200 dark:md:border-slate-700 animate-in fade-in slide-in-from-bottom-4 duration-200">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-amber-500 to-amber-600 text-white md:rounded-t-2xl shadow-sm">
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-slate-50 to-white dark:from-slate-800 dark:to-slate-900 custom-scrollbar relative">
        {/* Background Logo */}
        ...
        {/* Messages Container - z-10 for overlapping */}
        <div className="relative z-10 space-y-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center text-slate-400 py-8 min-h-[300px]">
              ...
              <p className="font-semibold text-slate-600 dark:text-slate-300">No hay mensajes todavía</p>
                ...
                >
                  ...
                  <div
                    className={`max-w-[75%] ${isOwn
                      ...
                      : "bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-2xl rounded-bl-md border border-slate-200 dark:border-slate-700 shadow-sm"
      ...
      {/* Input */}
      <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-900 md:rounded-b-2xl">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-full focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent bg-slate-50 dark:bg-slate-800 dark:text-slate-100 transition-all"
            maxLength={500}
          />
          <button
            onClick={handleSend}
