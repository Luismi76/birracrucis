...
export default function SplashScreen({
  ...
  return (
    <div
      className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center overflow-hidden transition-opacity duration-700 ease-out bg-[#FFFBEB] ${fadeOut ? "opacity-0 pointer-events-none" : "opacity-100"
        }`}
    >
      <style jsx>{`
        @keyframes fall {
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <p className="text-orange-100 font-medium text-lg mb-4 tracking-wide opacity-90">
          {theme === 'christmas' ? '¡Felices Fiestas!' :
            theme === 'semana-santa' ? 'Pasión por la cerveza' :
              theme === 'feria' ? '¡Vámonos de Feria!' :
                'La ruta definitiva'}
        </p>
      </div>
    </div>
